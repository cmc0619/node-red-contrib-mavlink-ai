...
const { requireEnumMember, enumMembers } = require('../protocol/protocol-values');
...
function knownModes(context = {}) {
  ...
  if (firmware === 'ardupilot') {
    ...
    return enumName
      ? enumMembers(enums, enumName, { dialect, consumer: 'flight-mode' }).map(({ name }) => name)
      : [];
  }
  return [];
}
...
function modeChoices(context = {}) {
  ...
  if (firmware === 'ardupilot') {
    const enumName = ARDUPILOT_MODE_ENUM[vehicleType];
    return enumName ? enumMembers(enums, enumName, { dialect, consumer: 'flight-mode' }) : [];
  }
  return [];
}
...
function resolveFlightMode(context = {}, modeName) {
  const { firmware, vehicleType, enums, dialect } = context;
  const enumContext = { dialect, consumer: 'flight-mode' };
  const baseMode = () => requireEnumMember(enums, 'MavModeFlag', 'CUSTOM_MODE_ENABLED', enumContext);
  ...
  if (firmware === 'ardupilot') {
    ...
    const names = knownModes(context);
    if (typeof modeName === 'string' && names.includes(modeName)) {
      return {
        base_mode: baseMode(),
        custom_mode: requireEnumMember(enums, enumName, modeName, enumContext)
      };
    }
    throw new MavlinkError(
      'UNKNOWN_MODE',
...
function modeNameForCustomMode(context = {}, customMode) {
  ...
  if (firmware === 'ardupilot') {
    ...
    for (const { name, value: mode } of enumMembers(enums, enumName, { dialect, consumer: 'flight-mode' })) {
      if (mode === value) {
        return name;
      }
    }
