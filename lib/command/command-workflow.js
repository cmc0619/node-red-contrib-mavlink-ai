...
class CommandSend {
   ...
   * @param {object} [opts.enums]           dialect enum index; used only to
   *   resolve a command *name* (a numeric command id needs none). The MavResult
   *   ACK values resolve from the always-available core bundle, so construction
   *   never depends on a loaded dialect.
   * @param {string} [opts.dialect]         dialect name, for error context only
  ...
  constructor(opts = {}) {
    ...
    /**
     * Our own identity (the resolved Local Identity's source system/
     * component, #228). Used to reject a COMMAND_ACK addressed to a different
     * GCS when two GCSs share one link and issue the same command to the same
     * vehicle (#99).
     */
    ...
    // MavResult is a common core enum — resolve ACCEPTED/IN_PROGRESS from the
    // always-available core bundle so a command send's ACK matching works even
    // when the profile has no loaded dialect (#309 review).
    const value = coreEnumValues({ consumer: 'command' });
    this.acceptedResult = value('MavResult', 'ACCEPTED');
    this.inProgressResult = value('MavResult', 'IN_PROGRESS');
      ...
      try {
        ...
        this._subs.push(
          this.connection.subscribe(
            { messageNames: ['COMMAND_ACK'], sysids: [this.targetSystem] },
  ...
  _onAck(payload) {
    ...
    if (result === this.inProgressResult) {
      /**
