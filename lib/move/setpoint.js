...
const coreValue = coreEnumValues({ consumer: 'move' });
const mask = (...members) => members.reduce((value, member) => value | coreValue('PositionTargetTypemask', member), 0);
...
const DIM_BITS = {
  ...
  position: mask('X_IGNORE', 'Y_IGNORE', 'Z_IGNORE'),
  /** vx/vy/vz — bits 3-5. */
  velocity: mask('VX_IGNORE', 'VY_IGNORE', 'VZ_IGNORE'),
  /** afx/afy/afz — bits 6-8. */
  accel: mask('AX_IGNORE', 'AY_IGNORE', 'AZ_IGNORE'),
  /** yaw — bit 10. */
  yaw: coreValue('PositionTargetTypemask', 'YAW_IGNORE'),
  /** yaw_rate — bit 11. */
  yawRate: coreValue('PositionTargetTypemask', 'YAW_RATE_IGNORE')
};
 ...
 */
const FORCE_BIT = mask('FORCE_SET');
 ...
 *
 * @param {string} memberKey  a bare MavFrame member key, e.g. 'LOCAL_NED'
 * @param {object} [enums]    dialect enum index
 * @param {object} [context]  merged into the unavailable-enum error (e.g. { dialect })
 * @returns {number}
 */
function resolveFrame(memberKey, enums, context = {}) {
  /** MavFrame is a common core enum — fall back to the core bundle when the
   * profile has no loaded dialect so the standard frames still resolve (#309
   * review); a present dialect still wins for custom frames. */
  const value = enums
    ? bindEnumValues(enums, { ...context, consumer: 'move' })
    : coreEnumValues({ ...context, consumer: 'move' });
  return value('MavFrame', memberKey);
}
