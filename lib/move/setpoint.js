 ...
 */
const FORCE_BIT = 0b0000_0010_0000_0000;
 ...
 * Unknown firmwares ('generic'/'custom'/absent) produce no warnings.
 *
 * @param {object} opts
...
function setpointWarnings(opts = {}) {
  ...
  if (firmware === 'px4') {
    if (frameName === 'MAV_FRAME_GLOBAL_TERRAIN_ALT_INT') {
      warnings.push('PX4 does not support terrain-altitude position targets (MAV_FRAME_GLOBAL_TERRAIN_ALT_INT).');
    }
    if (frameName === 'MAV_FRAME_LOCAL_OFFSET_NED' || frameName === 'MAV_FRAME_BODY_OFFSET_NED') {
      warnings.push(`PX4 does not support the OFFSET frames (${frameName}) — use MAV_FRAME_LOCAL_NED or MAV_FRAME_BODY_FRD.`);
    }
  }
  return warnings;
}
