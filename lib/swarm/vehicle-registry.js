...
class VehicleRegistry {
  ...
  constructor(opts = {}) {
    ...
    const enumContext = { dialect: this.dialect, consumer: 'vehicle-registry' };
    this.gcsType = requireEnumMember(this.enums, 'MavType', 'GCS', enumContext);
    this.armedFlag = requireEnumMember(this.enums, 'MavModeFlag', 'SAFETY_ARMED', enumContext);
    this.autopilotFirmware = new Map([
      [requireEnumMember(this.enums, 'MavAutopilot', 'ARDUPILOTMEGA', enumContext), 'ardupilot'],
      [requireEnumMember(this.enums, 'MavAutopilot', 'PX4', enumContext), 'px4']
    ]);
    this.typeVehicle = new Map(
      Object.entries(TYPE_FAMILY_MEMBERS).map(([member, family]) => [
        requireEnumMember(this.enums, 'MavType', member, enumContext),
        family
      ])
    );
