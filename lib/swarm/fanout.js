...
const { finite, nedOffsetToGlobal } = require('./coordinate-frames');
...
/**
 * Fail fast on a bad command/frame at the fan-out node, matching the Mission
 * node. An out-of-range id, a non-integer, or an unknown enum-NAME string throws
 * `INVALID_FIELD` here rather than being deferred to the codec — which in the
 * default build-only fan-out surfaces late in the downstream Out node, decoupled
 * from the fan-out node the operator is watching (it shows a green "fan-out N").
 * An in-range integer passes: the deliberate numeric escape hatch, identical to
 * Mission. With no dialect (`enums` null) it falls back to an integer-or-non-
 * blank-name shape check. Frame=50 and other valid-but-undefined enum ids still
 * pass by design — this closes the *late/decoupled error*, not that escape hatch.
 *
 * @param {*} value
 * @param {string} field     'command' or 'frame' (for the error message)
 * @param {string} enumName  'MavCmd' or 'MavFrame'
 * @param {number} max       inclusive uint max (0xffff for command, 0xff frame)
 * @param {?object} enums    dialect enum index, or null when unavailable
 * @param {number} [sysid]   error context
 * @returns {void}
 * @throws {MavlinkError} INVALID_FIELD
 */
function validateEnumField(value, field, enumName, max, enums, sysid) {
  const numeric = typeof value === 'number' || (typeof value === 'string' && /^-?\d+$/.test(value.trim()));
  if (numeric) {
    const n = Number(value);
    if (!Number.isInteger(n) || n < 0 || n > max) {
      throw new MavlinkError('INVALID_FIELD', `Fan-out ${field} must be an integer 0..${max} (got ${JSON.stringify(value)}).`, {
        field,
        value,
        sysid
      });
  ...
  if (enums) {
    const resolved = enumResolver.resolveInEnum(enums, enumName, value);
    if (!Number.isInteger(Number(resolved))) {
      throw new MavlinkError('INVALID_FIELD', `Fan-out ${field} '${value}' is not a known ${enumName} in the active dialect.`, {
        field,
        value,
        sysid
      });
    ...
    throw new MavlinkError('INVALID_FIELD', `Fan-out ${field} must be a ${enumName} integer or name (got ${JSON.stringify(value)}).`, {
      field,
      value,
      sysid
    });
...
function buildTargetMessage(opts) {
  const { command, useInt, base = {}, target, origin, defaults = {}, enums = null } = opts;
  ...
  if (useInt) {
     ...
     * reposition every vehicle to 0°N 0°E. An EXPLICIT caller-supplied param5/6
     * still wins, checked by presence rather than truthiness: 0 is the equator/
     * prime meridian, not "unset".
     */
    const altOnly = position !== null && position.lat === null;
    const explicitParam5 = target.param5 !== undefined || base.param5 !== undefined;
    const explicitParam6 = target.param6 !== undefined || base.param6 !== undefined;
    const x =
      target.x !== undefined
        ? requireInt32(finiteParam(target.x, 'x', target.sysid), 'x', target.sysid)
        : position && position.lat !== null
          ? degToDegE7(finite(position.lat, 'position.lat'))
          : explicitParam5
    ...
    const y =
      target.y !== undefined
        ...
        : position && position.lon !== null
          ? degToDegE7(finite(position.lon, 'position.lon'))
          : explicitParam6
    ...
    const z =
      target.z !== undefined
        ...
        : position && position.alt !== null
          ? position.alt
          : params.param7;
    const frame = target.frame !== undefined ? target.frame : base.frame !== undefined ? base.frame : 'MAV_FRAME_GLOBAL';
    // COMMAND_INT frame is a uint8 MAV_FRAME — fail fast on a bad one here, like
    // the Mission node, instead of deferring to the codec (see validateEnumField).
    validateEnumField(frame, 'frame', 'MavFrame', 0xff, enums, target.sysid);
    fields = {
      command,
      frame,
      current: 0,
    ...
    if (position) {
      if (position.lat !== null) {
         ...
         * caller-supplied param5/6 still win — presence-checked, not
         * truthiness-checked, so an explicit 0 (equator / prime meridian)
         * stays a coordinate.
         */
        if (target.param5 === undefined && base.param5 === undefined) {
          params.param5 = NaN;
        }
        if (target.param6 === undefined && base.param6 === undefined) {
          params.param6 = NaN;
        }
      }
...
function buildFanout(opts) {
  const { command, broadcast, enums = null } = opts;
  if (command === undefined || command === null || command === '') {
    throw new MavlinkError('NO_COMMAND', 'Fan-out needs a command (MAV_CMD name or number).');
  }
  // MAV_CMD rides COMMAND_LONG/COMMAND_INT as uint16. Fail fast on a bad command
  // (out of range, non-integer, or unknown name) for the whole fan-out.
  validateEnumField(command, 'command', 'MavCmd', 0xffff, enums);
  if (broadcast) {
    return [
      buildTargetMessage({
        command,
