...
const { nedOffsetToGlobal, degToDegE7 } = require('./coordinate-frames');
...
function resolvePosition(target, origin) {
  ...
  if (target.lat !== undefined || target.lon !== undefined) {
    ...
    return { lat: Number(target.lat), lon: Number(target.lon), alt: target.alt !== undefined ? Number(target.alt) : null };
  }
  if (hasOffset) {
  ...
  if (target.alt !== undefined) {
    return { lat: null, lon: null, alt: Number(target.alt) };
  }
  return null;
}
...
function buildTargetMessage(opts) {
  ...
  for (let i = 1; i <= 7; i += 1) {
    const key = `param${i}`;
    params[key] = target[key] !== undefined ? Number(target[key]) : base[key] !== undefined ? Number(base[key]) : 0;
  }
      ...
      target.y !== undefined
        ...
        : position && position.lon !== null
          ...
          : params.param6
            ...
            : 0;
    const z =
      target.z !== undefined ? Number(target.z) : position && position.alt !== null ? position.alt : params.param7;
    if (!Number.isFinite(x) || !Number.isFinite(y) || !Number.isFinite(Number(z))) {
      throw new MavlinkError('BAD_COORDINATES', `COMMAND_INT coordinates for sysid ${target.sysid} must be numeric (x=${x}, y=${y}, z=${z}).`, {
        sysid: target.sysid
      });
    }
    fields = {
