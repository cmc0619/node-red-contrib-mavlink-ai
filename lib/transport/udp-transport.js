'use strict';

const dgram = require('dgram');
const { EventEmitter } = require('events');
const { MavlinkError } = require('../util/errors');
const { toInt } = require('../util/validation');
const { boundedWrite, DEFAULT_WRITE_TIMEOUT_MS } = require('./bounded-write');
const { scheduleReconnect } = require('./reconnect-backoff');

/**
 * UDP transport (DESIGN.md §17.1, condensed in #243).
 *
 * Modeless: every socket is a peer. It binds (an explicit port, or an
 * ephemeral one when blank), learns validated senders, and sends to the fixed
 * remote when one is configured, else to learned peers.
 *
 * Peer tracking (issue #21): besides the most recent sender, the socket keeps a
 * per-sysid endpoint map (keyed by the validated packet's header sysid) so
 * that with multiple vehicles on one port, an outbound packet addressed to a
 * specific target_system goes to the endpoint that owns that sysid instead of
 * whichever vehicle spoke last. A manual remote host/port override wins.
 *
 * Trust boundary (#85, #239): receiving a datagram commits nothing. Routing
 * state changes only when the connection calls
 * {@link UdpTransport#confirmPeer} with the source endpoint of a packet that
 * passed CRC/framing, route acceptance, and (when enabled) signature
 * verification — the endpoint travels with the validated packet itself
 * (carried per-read through the decoder, #239) instead of being looked up in
 * a mutable latest-claimant table, so a malformed, route-rejected,
 * signature-rejected, or merely *racing* datagram can never replace the
 * fallback peer or redirect a sysid's traffic.
 *
 * Events: 'listening', 'data' (Buffer, rinfo), 'sendPartialFailure', 'error'
 */

/**
 * Validate a resolved UDP destination before handing it to the socket, so a
 * configuration mistake surfaces as a clear project error instead of a lower
 * level runtime failure. Returns a {@link MavlinkError} for an invalid target,
 * or null when the destination is usable.
 *
 * @param {{address: *, port: *}} target
 * @returns {?MavlinkError}
 */
function validateDestination(target) {
  const { address, port } = target;
  if (typeof address !== 'string' || address.trim() === '') {
    return new MavlinkError(
      'UDP_INVALID_DEST',
      'UDP destination address must be a non-empty string.',
      { address, port }
    );
  }
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    return new MavlinkError(
      'UDP_INVALID_DEST',
      'UDP destination port must be an integer in [1, 65535].',
      { address, port }
    );
  }
  return null;
}

/**
 * Enable SO_BROADCAST on a bound socket. Without it, a broadcast destination
 * (e.g. remote host 192.168.1.255 for a SITL/router setup) makes every send
 * fail with EACCES and no hint. Standard MAVLink tooling (pymavlink, MAVProxy)
 * enables it unconditionally; harmless for unicast destinations.
 *
 * @param {import('dgram').Socket} socket
 * @returns {void}
 */
function enableBroadcast(socket) {
  try {
    socket.setBroadcast(true);
  } catch {
    /** Not fatal for unicast use; broadcast sends will fail loudly instead. */
  }
}

/**
 * Whether a sysid is in the ground-station range. MAVLink convention puts GCSs
 * at the top of the sysid space (255 canonical; 250+ in practice) and vehicles
 * at the low end, so this heuristic keeps a GCS from stealing the fallback peer
 * from a vehicle (#148). It only gates the fallback endpoint — per-sysid routing
 * and broadcast fan-out are unaffected.
 *
 * @param {*} sysid
 * @returns {boolean}
 */
function isGcsSysid(sysid) {
  const n = Number(sysid);
  return Number.isFinite(n) && n >= 250;
}

/**
 * De-duplicate peer endpoints by address:port, preserving order, so a broadcast
 * fan-out never sends the same datagram to one endpoint twice (#148).
 *
 * @param {Iterable<{address: string, port: number}>} peers
 * @returns {object[]}
 */
function dedupeEndpoints(peers) {
  const seen = new Set();
  const out = [];
  for (const peer of peers) {
    const key = `${peer.address}:${peer.port}`;
    if (!seen.has(key)) {
      seen.add(key);
      out.push(peer);
    }
  }
  return out;
}

class UdpTransport extends EventEmitter {
  constructor(config = {}) {
    super();
    this.bindAddress = config.bindAddress || '0.0.0.0';
    // Blank/0 bind port means "ephemeral" (#243): a send-first socket that is
    // reached only via replies. The presence rules in transport-fields.js
    // guarantee a remote pair exists in that case.
    this.bindPort = toInt(config.bindPort, 0);
    this.remoteHost = config.remoteHost || '';
    this.remotePort = toInt(config.remotePort, 0);
    /**
     * Whether a send can be delivered to a SUBSET of peers keyed by sysid
     * (meta.sysids). True only in peer-learning mode: a fixed remote
     * destination is one shared endpoint, so per-sysid delivery is
     * impossible there — as it is on serial/tcp streams, which never set
     * this flag. The connection's mixed-version broadcast split (#199) is
     * gated on it, because delivering both version encodings to a shared
     * medium would double-deliver to MAVLink2 peers (they parse v1 too).
     */
    this.canRouteBySysid = !(this.remoteHost && this.remotePort);
    this.reconnect = config.reconnect !== false;
    this.reconnectDelayMs = Number(config.reconnectDelayMs || 2000);
    /** Completion deadline per datagram send (#237): a wedged socket whose send
     * callback never fires must not hold the shared outbound drain loop open. */
    this.writeTimeoutMs = Number(config.writeTimeoutMs || DEFAULT_WRITE_TIMEOUT_MS);
    this.socket = null;
    /**
     * True only between a successful bind ('listening') and the socket dropping
     * to null (error/close/stop). dgram has no public listening flag, so this
     * tracks it: start() assigns this.socket synchronously, but a send in the
     * window before bind() completes (or one that is about to fail with
     * EADDRINUSE) must not fall through to the transient UDP_NO_PEER a
     * fire-and-forget sender silently waits out.
     */
    this._bound = false;
    /** { address, port } — fallback endpoint for a send with no known per-sysid
     * peer. Prefers a vehicle over a GCS-range sysid (see confirmPeer, #148). */
    this.learnedPeer = null;
    /** sysid that owns the current learnedPeer, so a later GCS confirmation
     * can't steal the fallback from a vehicle (#148). */
    this._learnedPeerSysid = null;
    /** sysid -> { address, port } (validated). */
    this.peersBySysid = new Map();
    this._closing = false;
    this._reconnectTimer = null;
    /**
     * Grows the backoff delay per consecutive failed bind; reset once the
     * socket successfully starts listening (#149).
     */
    this._reconnectAttempts = 0;
  }

  /**
   * Transport descriptor attached to decoded messages (§14.1 `transport`).
   *
   * @returns {{name: string, type: string, bindAddress: string, bindPort: number,
   *   remoteHost: ?string, remotePort: ?number}}
   */
  get descriptor() {
    return {
      name: this.name,
      type: 'udp',
      bindAddress: this.bindAddress,
      bindPort: this.bindPort,
      remoteHost: this.remoteHost || (this.learnedPeer && this.learnedPeer.address),
      remotePort: this.remotePort || (this.learnedPeer && this.learnedPeer.port)
    };
  }

  /**
   * Open the UDP socket and begin emitting `data`/`peer`/`listening` events.
   * A blank bind port binds an ephemeral one (send-first socket).
   *
   * @returns {void}
   */
  start() {
    this._closing = false;
    // No reuseAddr: with SO_REUSEADDR two UDP sockets (even in different
    // processes) can silently bind the same port — including a port picked by
    // bind(0) — and inbound datagrams are then split between them at random.
    // UDP has no TIME_WAIT, so reuseAddr buys nothing on redeploy; a genuine
    // double-bind should fail loudly with EADDRINUSE instead of quietly
    // stealing half the traffic.
    const socket = dgram.createSocket({ type: 'udp4' });
    this.socket = socket;

    socket.on('error', (err) => {
      this.emit('error', new MavlinkError('UDP_ERROR', err.message, { bindPort: this.bindPort }));
      /**
       * A bind failure (EADDRINUSE — common when a redeploy races the previous
       * socket's release) or a fatal socket error leaves a dead socket. Close it
       * and retry with backoff instead of staying permanently dead until a
       * manual redeploy (#149). Without this the socket that couldn't bind is
       * also never closed, leaking the handle.
       *
       * Not gated on `this.reconnect`: the editor hides the Reconnect control
       * for all udp modes, so a saved flow can carry a stale hidden
       * `reconnect:false` the user can't see or change; gating on it would
       * silently disable bind recovery for exactly those flows. Recovering a
       * failed bind is startup resilience, distinct from the post-drop client
       * reconnect that the toggle governs.
       */
      if (this._closing || this.socket !== socket) {
        return;
      }
      this.socket = null;
      this._bound = false;
      socket.removeAllListeners();
      socket.on('error', () => {});
      try {
        socket.close();
      } catch {
        /** the socket is already unusable — nothing to close */
      }
      this._scheduleReconnect();
    });

    socket.on('message', (buffer, rinfo) => {
      /**
       * Receipt commits nothing (#85, #239): the datagram's source endpoint
       * rides along on the 'data' event, and only comes back — bound to the
       * specific packet it carried — when the connection validates that packet
       * and calls confirmPeer().
       */
      this.emit('data', buffer, rinfo);
    });

    socket.on('listening', () => {
      this._reconnectAttempts = 0;
      this._bound = true;
      enableBroadcast(socket);
      this.emit('listening', socket.address());
    });
    socket.bind(this.bindPort, this.bindAddress);
  }

  /**
   * Schedule a rebind attempt after a bind/socket failure (#149): shared
   * exponential backoff + jitter, one policy for all transports (see
   * reconnect-backoff).
   *
   * @returns {void}
   */
  _scheduleReconnect() {
    scheduleReconnect(this, 'UDP_ERROR', () => ({ bindPort: this.bindPort }));
  }

  /**
   * Commit a validated packet's own source endpoint as a trusted, routable
   * peer (#85, #239). Called by the connection after an inbound packet from
   * `sysid` has passed framing/CRC, route acceptance, and any signature
   * verification policy — with the endpoint of the datagram that carried that
   * very packet. Binding the endpoint to the packet (instead of reading a
   * mutable "latest claimant per sysid" table at confirm time) closes the
   * race where a forged datagram arriving between receipt and validation
   * could be the endpoint promoted (#239). An invalid or missing endpoint is
   * ignored — trust is never restored from anything but a validated read.
   *
   * @param {number} sysid  source system of the validated packet
   * @param {{address: string, port: number}} endpoint  source endpoint of the
   *   datagram that carried the validated packet
   * @returns {void}
   */
  confirmPeer(sysid, endpoint) {
    if (sysid == null) {
      return;
    }
    const sys = Number(sysid);
    if (!Number.isInteger(sys) || !endpoint || validateDestination(endpoint)) {
      return;
    }
    /** Copy: the stored peer must not alias an object a caller can mutate. */
    const peer = { address: endpoint.address, port: endpoint.port };
    this.peersBySysid.set(sys, peer);
    /**
     * Update the fallback endpoint, but don't let a GCS-range sysid steal it
     * from a vehicle (#148): a second GCS on the port would otherwise capture
     * untargeted traffic (including our own heartbeat). Replace when there is no
     * fallback yet, when the same sysid re-confirms (its endpoint moved), or as
     * long as we're not demoting a vehicle fallback to a GCS one.
     */
    const replace =
      !this.learnedPeer ||
      this._learnedPeerSysid === sys ||
      !(isGcsSysid(sys) && !isGcsSysid(this._learnedPeerSysid));
    if (replace) {
      this.learnedPeer = peer;
      this._learnedPeerSysid = sys;
    }
  }

  /**
   * Resolve the list of endpoints a send should reach (#148):
   *
   * - a manual remote host/port override is the single destination;
   * - a specific nonzero `target_system` unicasts to that sysid's learned
   *   endpoint (falling back to `learnedPeer` when that sysid isn't known yet);
   * - a broadcast (`target_system` 0) or an untargeted send (no `targetSystem`)
   *   fans out to *every* learned peer — MAVLink routing delivers a broadcast to
   *   all known links, so with two vehicles on one port both must receive it,
   *   and our own heartbeat must reach every vehicle rather than only the last
   *   sender. Falls back to `learnedPeer` while no per-sysid peer is known.
   *
   * Endpoints are de-duplicated by address:port so peers sharing an endpoint
   * aren't sent to twice.
   *
   * @param {object} [meta]  { targetSystem } or { sysids } (version group, #199)
   * @returns {object[]} zero or more { address, port }
   */
  _targets(meta) {
    if (this.remoteHost && this.remotePort) {
      return [{ address: this.remoteHost, port: this.remotePort }];
    }
    /**
     * A version-group send (#199): the connection encoded this buffer for
     * exactly these sysids (a v1 or v2 subset of a mixed fleet), so it goes
     * to their learned endpoints only — no learnedPeer fallback, and no
     * fan-out to peers of the other wire version, which would just discard
     * the frame. Unknown sysids are skipped; an empty result surfaces as
     * UDP_NO_PEER in send().
     */
    if (meta && Array.isArray(meta.sysids)) {
      const endpoints = [];
      for (const sysid of meta.sysids) {
        const peer = this.peersBySysid.get(Number(sysid));
        if (peer) {
          endpoints.push(peer);
        }
      }
      return dedupeEndpoints(endpoints);
    }
    const targetSystem = meta && meta.targetSystem != null ? Number(meta.targetSystem) : null;
    if (targetSystem != null && targetSystem !== 0) {
      const peer = this.peersBySysid.get(targetSystem);
      if (peer) {
        return [peer];
      }
      return this.learnedPeer ? [this.learnedPeer] : [];
    }
    if (this.peersBySysid.size > 0) {
      return dedupeEndpoints(this.peersBySysid.values());
    }
    return this.learnedPeer ? [this.learnedPeer] : [];
  }

  /**
   * True when no learned endpoint is shared between the two sysid groups.
   * The connection's mixed-version broadcast split (#199) requires this:
   * behind a MAVLink router/bridge a v1 sysid and a v2 sysid can be learned
   * from the SAME address:port, and delivering both version encodings there
   * would double-deliver to MAVLink2 peers (they parse v1 frames too).
   * Unknown sysids have no endpoint and contribute nothing.
   *
   * @param {number[]} sysidsA
   * @param {number[]} sysidsB
   * @returns {boolean}
   */
  sysidEndpointsDisjoint(sysidsA, sysidsB) {
    const seen = new Set();
    for (const sysid of sysidsA) {
      const peer = this.peersBySysid.get(Number(sysid));
      if (peer) {
        seen.add(`${peer.address}:${peer.port}`);
      }
    }
    for (const sysid of sysidsB) {
      const peer = this.peersBySysid.get(Number(sysid));
      if (peer && seen.has(`${peer.address}:${peer.port}`)) {
        return false;
      }
    }
    return true;
  }

  /**
   * Send a buffer to the resolved target.
   *
   * @param {Buffer} buffer
   * @param {object} [meta]  { targetSystem } routes to that sysid's endpoint;
   *   { sysids } routes to exactly those sysids' endpoints (#199)
   * @returns {Promise<void>}
   */
  send(buffer, meta) {
    /**
     * A socket that is null (never started / already torn down) or not yet
     * `_bound` (start() assigns this.socket synchronously, but the async bind()
     * may still be in flight — or about to fail with EADDRINUSE) is not a place
     * a send can land — often a permanent failure (a bind that lost the port).
     * Reject it with a distinct code *before* the empty-target check so a
     * not-started listener never masquerades as the transient UDP_NO_PEER a
     * fire-and-forget sender silently waits out; UDP_NO_PEER can then only mean
     * "bound and listening, no peer learned yet."
     */
    if (!this.socket || !this._bound) {
      return Promise.reject(new MavlinkError('UDP_NOT_STARTED', 'UDP socket is not started.'));
    }
    const targets = this._targets(meta);
    if (targets.length === 0) {
      return Promise.reject(
        new MavlinkError('UDP_NO_PEER', 'No UDP peer to send to yet. Set a remote host/port or wait for an inbound packet.')
      );
    }
    /**
     * Fan out to every resolved endpoint. A broadcast/untargeted send is
     * best-effort: it resolves as long as one datagram went out and only
     * rejects when *every* endpoint failed, so one dead peer can't fail a
     * broadcast to healthy ones (#148). A unicast has a single endpoint, so
     * this preserves the original one-target success/failure semantics.
     */
    const sends = targets.map((target) => this._sendOne(buffer, target));
    return Promise.allSettled(sends).then((results) => {
      const rejected = results.filter((r) => r.status === 'rejected');
      /**
       * A partial fan-out failure (some peers sent, some failed) still counts
       * as success — the broadcast went out — but a dead/degraded peer would
       * otherwise be invisible. Surface it on a dedicated, non-'error' event so
       * an operator can observe it without an unlistened 'error' throwing.
       */
      if (rejected.length > 0 && rejected.length < results.length) {
        this.emit('sendPartialFailure', {
          failed: rejected.length,
          total: results.length,
          reasons: rejected.map((r) => r.reason)
        });
      }
      if (results.length > rejected.length) {
        return;
      }
      throw results[0].reason;
    });
  }

  /**
   * Send the buffer to one validated endpoint.
   *
   * @param {Buffer} buffer
   * @param {object} target  { address, port }
   * @returns {Promise<void>}
   */
  _sendOne(buffer, target) {
    const invalid = validateDestination(target);
    if (invalid) {
      return Promise.reject(invalid);
    }
    /**
     * Bounded send (#237): dgram callbacks normally fire immediately, but a
     * wedged socket/interface can leave one pending forever — and that single
     * stalled send would block the shared outbound drain loop until the queue
     * fills. Late callbacks after the deadline are ignored (settle once).
     */
    const socket = this.socket;
    return boundedWrite({
      write: (cb) => socket.send(buffer, target.port, target.address, cb),
      timeoutMs: this.writeTimeoutMs,
      timeoutError: () => new MavlinkError('UDP_SEND_TIMEOUT', `UDP send did not complete within ${this.writeTimeoutMs} ms.`, target),
      wrapError: (err) => new MavlinkError('UDP_SEND_FAILED', err.message, target)
    });
  }

  /**
   * Close the socket and clear the learned peer.
   *
   * @returns {Promise<void>} resolves once the socket is closed
   */
  stop() {
    return new Promise((resolve) => {
      this._closing = true;
      this._bound = false;
      clearTimeout(this._reconnectTimer);
      this._reconnectTimer = null;
      this.learnedPeer = null;
      this._learnedPeerSysid = null;
      this.peersBySysid.clear();
      if (!this.socket) {
        resolve();
        return;
      }
      const socket = this.socket;
      this.socket = null;
      socket.removeAllListeners();
      /**
       * Keep a no-op 'error' handler through the async close so a late socket
       * error during close() doesn't hit a listener-less emitter and crash the
       * process with an uncaughtException (#149).
       */
      socket.on('error', () => {});
      try {
        socket.close(() => resolve());
      } catch {
        resolve();
      }
    });
  }
}

module.exports = { UdpTransport, validateDestination };
