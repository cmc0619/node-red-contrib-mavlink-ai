...
class UdpTransport extends EventEmitter {
  ...
  /**
   * The learned endpoint identity for a sysid, as an opaque comparable key,
   * or null while that sysid has no learned endpoint. The connection's
   * mixed-version broadcast split (#199) uses this to detect version groups
   * sharing an endpoint (a MAVLink router/bridge) — such an endpoint must
   * receive exactly one frame, never both version encodings.
   *
   * @param {number} sysid
   * @returns {?string}
   */
  endpointKeyForSysid(sysid) {
    const peer = this.peersBySysid.get(Number(sysid));
