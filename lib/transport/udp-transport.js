...
class UdpTransport extends EventEmitter {
  ...
  send(buffer, meta) {
    ...
    if (targets.length === 0) {
      /**
       * udp-out never learns peers, so an empty target set is a *permanent*
       * misconfiguration (the remote endpoint isn't set), not the transient
       * "no peer yet" a udp-peer sees before an inbound packet arrives. Use a
       * distinct code so a fire-and-forget sender surfaces the config error
       * instead of quietly waiting for a peer that can never appear.
       */
      if (this.mode === 'udp-out') {
        return Promise.reject(
          new MavlinkError('UDP_NO_REMOTE', 'Transport udp-out requires a remote host and a remote port (1-65535).')
        );
      }
      return Promise.reject(
        new MavlinkError('UDP_NO_PEER', 'No UDP peer to send to yet. Set a remote host/port or wait for an inbound packet (udp-peer).')
      );
    }
    /**
