...
class UdpTransport extends EventEmitter {
  ...
  send(buffer, meta) {
    ...
    /**
     * A null socket means the transport never started or already tore down —
     * often a permanent failure (a bind that lost the port). Reject it with a
     * distinct code *before* the empty-target check so a not-started listener
     * never masquerades as the transient UDP_NO_PEER a fire-and-forget sender
     * silently waits out; UDP_NO_PEER can then only mean "bound and listening,
     * no peer learned yet."
     */
    if (!this.socket) {
