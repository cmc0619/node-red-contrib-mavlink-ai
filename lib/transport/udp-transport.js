...
class UdpTransport extends EventEmitter {
  constructor(config = {}) {
    ...
    this.socket = null;
    /** { address, port } — most recent *validated* sender. */
  ...
  start() {
    ...
    socket.on('error', (err) => {
       ...
       * and, when reconnect is enabled, retry with backoff instead of staying
       * permanently dead until a manual redeploy (#149). Without this the socket
       * that couldn't bind is also never closed, leaking the handle.
       */
