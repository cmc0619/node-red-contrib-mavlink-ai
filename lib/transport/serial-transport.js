...
/** Cap on the exponential reopen backoff so retries stay responsive (#149). */
...
class SerialTransport extends EventEmitter {
  ...
  /**
   * Schedule a reopen attempt with exponential backoff (capped) plus jitter, so
   * a flapping device or a rebooting companion computer isn't retried in a tight
   * fixed-rate loop and multiple serial connections don't retry in lockstep
   * (#149).
   *
   * @returns {void}
   */
  _scheduleReconnect() {
    ...
    this._reconnectTimer = setTimeout(() => {
      ...
      /**
       * start() can throw synchronously (serialport constructor validation,
       * loadSerialPort() edge cases); inside a timer callback that would be an
       * uncaughtException killing the whole Node-RED process. Surface it as a
       * transport error and keep retrying with backoff instead.
       */
      try {
        this.start();
      } catch (err) {
        this.emit('error', new MavlinkError('SERIAL_ERROR', err && err.message ? err.message : String(err), { path: this.path }));
        this._scheduleReconnect();
      }
    }, delay);
