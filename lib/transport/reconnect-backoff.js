...
function scheduleReconnect(transport, errorCode, context) {
  ...
  /**
   * emit() is synchronous: a 'reconnecting' listener may have called stop(),
   * which sets _closing and clears the timer — re-arming here would leave a
   * live timer past teardown, violating the close lifecycle (CodeRabbit
   * review). Same guard after the error emit below.
   */
  if (transport._closing) {
    return;
  }
