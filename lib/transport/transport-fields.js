 ...
 * against whichever half is missing. A present remote port must also be a
 * usable destination (an integer 1..65535): port 0 would pass a pure presence
 * check but the transport coerces it to "no fixed destination", so sends fail
 * forever as the transient UDP_NO_PEER — hiding a permanent misconfiguration
 * (#243, Codex review). The bind port is not range-checked here: blank/0 bind
 * legitimately means "ephemeral".
 *
 * @param {object} config
 * @param {Array<{field: string, message: string}>} problems  appended in place
 * @returns {boolean} true when a complete, usable remote pair is present
 */
function checkRemotePair(config, problems) {
  ...
  if (hasPort) {
    ...
    if (!Number.isInteger(port) || port < 1 || port > 65535) {
      problems.push({ field: 'remotePort', message: 'Remote port must be an integer between 1 and 65535.' });
      return false;
    }
  }
