...
class TcpTransport extends EventEmitter {
  ...
  send(buffer) {
    return new Promise((resolve, reject) => {
      if (this.mode === 'tcp-server') {
        /**
         * A null server means listen() never succeeded or already tore down —
         * often a permanent failure (a port already in use). Reject it with a
         * distinct code *before* the no-client check so a listener that never
         * came up never masquerades as the transient TCP_NO_CLIENT a
         * fire-and-forget sender silently waits out; TCP_NO_CLIENT can then only
         * mean "listening, no client connected yet."
         */
        if (!this.server) {
