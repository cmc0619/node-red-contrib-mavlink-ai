 ...
 *
 * Modes:
 *   client role  connect out to host:port
 *   server role  listen on host:port, fan out to connected clients
 *
...
class TcpTransport extends EventEmitter {
  constructor(config = {}) {
    super();
    /**
     * Role is derived by the factory from field presence (#243): 'client'
     * dials out to a remote, 'server' accepts inbound on a bind port.
     */
    this.role = config.role || 'client';
    this.host = config.host || '127.0.0.1';
    /**
     * Not `config.port || 5760`: like the UDP bind port, an explicit 0 means
     * "ephemeral port" for the server role and must stay 0, not silently turn into
     * the default port.
     */
   ...
   */
  get descriptor() {
    return { name: this.name, type: 'tcp', role: this.role, remoteHost: this.host, remotePort: this.port };
  }

  /**
   * Start the transport in its derived role.
   *
   * @returns {void}
   */
  start() {
  ...
  _startServer() {
    ...
    server.on('error', (err) => {
       ...
       * the client role. Close the dead handle and reschedule so a transient port
       * conflict isn't permanent until a manual redeploy (#149).
       *
       * This bind recovery is deliberately NOT gated on `this.reconnect`: the
       * editor hides the Reconnect control for the server role (it's a client/serial
  ...
  /**
   * Schedule a reconnect attempt (client reconnect or server relisten) via the
   * shared backoff + jitter policy (#149, see reconnect-backoff). Retries
   * through `start()` so it restarts in the transport's current mode.
   *
   * @returns {void}
   */
  _scheduleReconnect() {
    scheduleReconnect(this, 'TCP_ERROR');
  }
