...
const { chainFieldEnums } = require('./field-enums');
...
/**
 * Merge order per dialect, following the bundled `mavlink-mappings` module
 * split. Note upstream XML inheritance actually runs the other way around for
 * `standard` (standard.xml includes common.xml, which includes minimal.xml);
 * the bundled `standard` module carries only AUTOPILOT_VERSION plus a few
 * enums, and MAVLink message ids are globally unique, so merging it into every
 * common-based chain reproduces upstream content and the order is not
 * load-bearing.
 */
...
/**
 * Bundled dialect bundles, cached per resolved dialect name. A bundle is
 * immutable once built — every consumer only reads registry/byName/enums/
 * fieldEnums — but building one re-merges the module registries and rebuilds
 * the enum index (~2 ms for ardupilotmega), and a profile config node loads
 * its dialect on every deploy, so many profiles sharing a dialect paid that
 * rebuild each. Custom XML compiles are deliberately NOT cached: the file can
 * change between deploys, and serving a stale compile would silently ignore
 * the user's edit.
 */
const bundledBundleCache = new Map();
 ...
 * @param {Map<string, Map<string, string>>} [fieldEnums]  class -> field ->
 *   enum association for encode-time enum scoping (#153); bundled chains parse
 *   it from the shipped declarations, custom-XML classes carry their own
 *   FIELD_ENUMS static instead
 * @returns {DialectBundle}
 */
function buildBundleFromModules(name, requested, modules, magicNumbers, fieldEnums) {
...
function loadDialect(name, opts = {}) {
  try {
    ...
    // basename still resolves to a bundled dialect via resolveDialectName.
    ...
    let bundle = bundledBundleCache.get(resolved);
    if (!bundle) {
       ...
       * whose declared payload length may be truncated. Cached under the
       * resolved name with `requested` canonicalized to it, so the cache entry
       * is independent of which alias first built it.
       */
      bundle = buildBundleFromModules(resolved, resolved, modules, mappings.MSG_ID_MAGIC_NUMBER, chainFieldEnums(chain));
      bundledBundleCache.set(resolved, bundle);
    }
    /**
     * `requested` echoes the caller's own name back for diagnostics (a
     * `custom` request that resolved to a bundled basename should report as
     * `custom`); the shared immutable innards ride along on a shallow copy.
     */
    return bundle.requested === name ? bundle : Object.assign({}, bundle, { requested: name });
  } catch (err) {
    const e =
