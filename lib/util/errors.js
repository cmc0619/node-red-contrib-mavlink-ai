 ...
 * for the other party to appear — a udp socket waiting to learn a peer, a TCP
 ...
 * permanently down and a send must surface it; TRANSPORT_NOT_READY, UDP_NOT_STARTED
 * and TCP_NOT_LISTENING (the socket/server is null — often a permanent failed
 * start, e.g. a port already in use); and QUEUE_CLEARED (a genuine cancel).
 * Swallowing any of those would hide an unrecoverable link from Catch nodes.
 * The transports reject a null socket/server with these not-started codes
 * *before* the waiting-state check, so UDP_NO_PEER / TCP_NO_CLIENT can only mean
 * "started and listening, waiting for the other side" — never a failed start.
 *
 * @type {Set<string>}
 */
 ...
 * UDP_NOT_STARTED, TCP_NOT_LISTENING, QUEUE_CLEARED). The heartbeat can silently
 * retry all of these — they are already surfaced elsewhere (the connection
 * reports disconnects and fatal start errors; QUEUE_CLEARED means it is
 * deactivating), and re-emitting them every tick would only spam. In particular
 * the not-started codes are included so a teardown race (a heartbeat firing
 * after the socket/server dropped to null) stays quiet, while a fire-and-forget
 * Out send uses the narrower {@link TRANSPORT_WAITING_CODES} so an unrecoverable
 * link still errors.
 *
 * @type {Set<string>}
 */
