...
/**
 * Wrap a status payload in the standard `{ topic, payload }` message envelope.
 *
 * @param {object} opts  see {@link statusPayload}
 * @returns {{topic: string, payload: object}}
 */
function statusMessage(opts) {
  return { topic: 'mavlink/status', payload: statusPayload(opts) };
}
