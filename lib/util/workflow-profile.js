...
function resolveWorkflowContext(connection, opts = {}) {
   ...
   * not shadow (#302 review).
   */
  const sysidNum = Number(targetSystem);
  const compidNum = Number(targetComponent);
  const unicastTarget =
    Number.isInteger(sysidNum) && sysidNum >= 1 && sysidNum <= 255 &&
    Number.isInteger(compidNum) && compidNum >= 0 && compidNum <= 255;
  if (unicastTarget) {
