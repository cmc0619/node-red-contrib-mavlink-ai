 ...
 *   degenerate connection lacking `resolveProfile()`.
 */
function resolveWorkflowContext(connection, opts = {}) {
  ...
  if (!explicit && typeof connection.getProfileForPacket === 'function') {
    /**
     * Route the addressed target to its owning profile so the workflow
     * speaks that system's dialect, not the default's. The target ids stay
     * as addressed — only protocol behavior (enums, firmware, mission
     * preferences) follows the routed profile.
     */
    ...
    if (isProfileNode(routed) && routed.isValid() && routed !== profile) {
      // Protocol behavior only (enums, firmware, mission preferences) — the
      // routed Vehicle Profile never changes the local outbound identity (#228).
      profile = routed;
