...
/**
 * True for a value that is absent or blank — undefined, null, an empty string,
 * or a whitespace-only string. A whitespace-only string must count as blank:
 * `Number(' ')` is `0` (finite), so without this a `' '` would coerce to a
 * "real" 0 and silently pass a fallback-or-validation gate meant to reject a
 * left-blank field (Codex #248 review).
 *
 * @param {*} value
 * @returns {boolean}
 */
...
/**
 * Parse a list of numeric ids STRICTLY: a non-empty token that is neither a
 * wildcard ("*" / "any" / "all" = no constraint) nor an integer in [0, max]
 * is REPORTED as invalid instead of being silently dropped (#193). The
 * fail-open parser this replaced turned `"1O"` into `[]` (accept everything)
 * and `"1,2x"` into `[1]` (silently narrowed) — a typo in a safety/routing
 * filter became wider access, the same fail-open class the RouteTable closed
 * in #71. Callers surface `invalid` as a structured configuration error and
 * fail closed.
 *
 * @param {*} value
