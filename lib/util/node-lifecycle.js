 ...
 * the idle badge. "invalid profile" takes precedence over a construct-time
 * config error, which in turn takes precedence over "missing connection".
 *
 * A caller may also record a construct-time config problem (malformed static
 * JSON, an unset Delivery selection, ...) on `node._configError` (#207, #308).
 * The refresh checks it on every call, not just once at construct time: an
 * unset Delivery needs no connection, so without this a redeploy of some
 * *unrelated* node would still fire `flows:started`, this refresh would see
 * nothing it thinks it needs, and it would silently clear the red badge back
 * to idle while the node stays just as broken. Set `node._configError` to any
 * truthy value before (or any time after) calling this; a caller that never
 * sets it is unaffected (`undefined` is falsy, so the refresh behaves exactly
 * as it did before this existed).
 *
 * @param {object} RED     the Node-RED runtime
 * @param {object} node    the flow node (`node.status`, `node._configError`,
 *   and the refs set below)
