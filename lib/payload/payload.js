...
function buildPayload(action, opts = {}) {
  ...
  // review): the standard payload actions (camera/gimbal/mount/gripper/winch)
  // and the MavCmd they carry are all common core enums, so they resolve and
  // keep working without a dialect; an action enum genuinely absent from core
  // would still fail closed with ENUM_VALUE_UNAVAILABLE. A present dialect wins.
