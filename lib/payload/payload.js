 ...
 * @returns {number}
 */
function resolveAction(table, name, enumName, opts, code, label) {
  ...
  return protocolValue(opts, enumName, member);
}

function protocolValue(opts, enumName, member) {
  return requireEnumMember(opts.enums, enumName, member, {
 ...
 */
function resolveCommand(member, opts) {
  return protocolValue(opts, 'MavCmd', member);
}
...
const BUILDERS = {
  ...
  camera_mode(opts) {
    const modeMember = CAMERA_MODES[String(opts.cameraMode || 'image').toLowerCase()];
    if (modeMember === undefined) {
      throw new MavlinkError('BAD_CAMERA_MODE', `Camera mode must be image/video/survey (got '${opts.cameraMode}').`);
    }
    return command('SET_CAMERA_MODE', { param2: protocolValue(opts, 'CameraMode', modeMember) });
  },
  ...
  gimbal_aim(opts) {
    return command('DO_MOUNT_CONTROL', {
      ...
      param7: protocolValue(opts, 'MavMountMode', MAV_MOUNT_MODE_MAVLINK_TARGETING)
    });
  },
  /**
  ...
  gimbal_manager_aim(opts) {
    return {
      ...
      fields: {
        flags: opts.yawLock ? protocolValue(opts, 'GimbalManagerFlags', GIMBAL_MANAGER_FLAGS_YAW_LOCK) : 0,
  ...
  gripper(opts) {
    return command('DO_GRIPPER', {
      ...
      param2: resolveAction(GRIPPER_ACTIONS, opts.action, 'GripperActions', opts, 'BAD_GRIPPER_ACTION', 'Gripper action')
    });
  },
  /**
  ...
  winch(opts) {
    return command('DO_WINCH', {
      param1: num(opts.instance, 1),
      param2: resolveAction(
        WINCH_ACTIONS,
        opts.winchAction || 'relax',
        'WinchActions',
        opts,
        'BAD_WINCH_ACTION',
        'Winch action'
  ...
  parachute(opts) {
    return command('DO_PARACHUTE', {
      param1: resolveAction(
        PARACHUTE_ACTIONS,
        opts.parachuteAction,
        'ParachuteAction',
        opts,
        'BAD_PARACHUTE_ACTION',
        'Parachute action'
      )
    });
  },
...
function buildPayload(action, opts = {}) {
  ...
  if (typeof built.fields.command === 'string') {
    built.fields.command = resolveCommand(built.fields.command, opts);
  }
