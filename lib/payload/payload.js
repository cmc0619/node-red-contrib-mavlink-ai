...
const BUILDERS = {
  ...
  camera_mode(opts) {
    ...
    return command('SET_CAMERA_MODE', { param2: opts.value('CameraMode', modeMember) });
  },
...
function buildPayload(action, opts = {}) {
  ...
  const value = bindEnumValues(opts.enums, { dialect: opts.dialect || 'unknown', consumer: 'payload' });
