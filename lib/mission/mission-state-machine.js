 ...
 * @param {object} enums  active dialect enum index
 * @param {object} [context] strict-resolution error context
 * @returns {number}
 * @throws {MavlinkError} BAD_MISSION_TYPE for unrecognized strings
 */
function missionTypeToNumber(value, enums, context = {}) {
  // Standard mission types (MISSION/FENCE/RALLY/ALL) are common core enums —
  // fall back to the core bundle when the profile has no loaded dialect (#309
  // review); a genuinely custom type still resolves against `enums` below.
  const enumValue = enums ? bindEnumValues(enums, context) : coreEnumValues(context);
  if (typeof value === 'number') {
    return value;
  }
  if (value == null) {
    return enumValue('MavMissionType', 'MISSION');
  }
  const s = String(value).trim().toLowerCase();
  if (Object.prototype.hasOwnProperty.call(MISSION_TYPE_MEMBERS, s)) {
    return enumValue('MavMissionType', MISSION_TYPE_MEMBERS[s]);
  }
  const resolved = enumResolver.resolveInEnum(enums, 'MavMissionType', value);
...
class MissionWorkflow {
  constructor(opts = {}) {
    ...
    // MavMissionResult/MavMissionType/MavFrame are common core enums — resolve
    // from the always-available core bundle so mission download/upload/clear
    // construct even when the profile has no loaded dialect (#309 review). The
    // dialect-specific mission *type* the caller requested still resolves
    // against `this.enums` via missionTypeToNumber below.
    const value = coreEnumValues({ consumer: 'mission' });
    this.missionAccepted = value('MavMissionResult', 'ACCEPTED');
    this.missionTypeMission = value('MavMissionType', 'MISSION');
    this.missionTypeAll = value('MavMissionType', 'ALL');
    this.globalFrames = new Set(GLOBAL_FRAME_MEMBERS.map((member) => value('MavFrame', member)));
    this.missionTypeName = opts.missionType;
    this.missionType = missionTypeToNumber(opts.missionType, this.enums, {
  ...
  _matchesMissionType(payload) {
    ...
    if (
      this.missionType !== this.missionTypeMission &&
      got === this.missionTypeMission &&
      payload.raw &&
      Number(payload.raw.magic) === 0xfe
    ) {
      this._fail(
        new MavlinkError(
  ...
  _send(name, fields) {
    ...
    /** Mission protocol traffic rides the NORMAL band, explicitly (#241). */
