...
function missionTypeToNumber(value, enums, context = {}) {
  ...
  if (value == null) {
    return requireEnumMember(enums, 'MavMissionType', 'MISSION', context);
  }
  const s = String(value).trim().toLowerCase();
  if (Object.prototype.hasOwnProperty.call(MISSION_TYPE_MEMBERS, s)) {
    return requireEnumMember(enums, 'MavMissionType', MISSION_TYPE_MEMBERS[s], context);
  }
...
class MissionWorkflow {
  constructor(opts = {}) {
    ...
    const protocolContext = { dialect: this.dialect, consumer: 'mission' };
    this.missionAccepted = requireEnumMember(this.enums, 'MavMissionResult', 'ACCEPTED', protocolContext);
    this.missionTypeMission = requireEnumMember(this.enums, 'MavMissionType', 'MISSION', protocolContext);
    this.missionTypeAll = requireEnumMember(this.enums, 'MavMissionType', 'ALL', protocolContext);
    this.globalFrames = new Set(
      GLOBAL_FRAME_MEMBERS.map((member) => requireEnumMember(this.enums, 'MavFrame', member, protocolContext))
    );
    this.missionTypeName = opts.missionType;
    this.missionType = missionTypeToNumber(opts.missionType, this.enums, protocolContext);
