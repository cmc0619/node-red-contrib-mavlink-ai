...
function missionTypeToNumber(value, enums, context = {}) {
  const enumValue = bindEnumValues(enums, context);
...
class MissionWorkflow {
  constructor(opts = {}) {
    ...
    // enums, so resolve them from the core defs (same as the module-load
    // constants). `this.enums` (the profile dialect) is used only for a
    // caller-requested custom mission *type* via missionTypeToNumber below.
