...
class MissionDownload extends MissionWorkflow {
  ...
  _requestItem(seq) {
    ...
    /**
     * A pre-INT MAVLink-1 autopilot never answers MISSION_REQUEST_INT, so the
     * first unanswered _INT request (a retry is now firing, _retries > 0) drops
     * us to the plain MISSION_REQUEST for the rest of the download rather than
     * timing out entirely (#145). MISSION_ITEM (float degrees) is handled by
     * extractItem the same as MISSION_ITEM_INT.
     */
    if (this.useInt && this._retries > 0 && !this._downgradedRequest) {
