...
class MissionDownload extends MissionWorkflow {
  ...
  _start() {
    ...
    this._subscribe(
      { messageNames: ['MISSION_COUNT', 'MISSION_ITEM_INT', 'MISSION_ITEM', 'MISSION_ACK'], sysids: [this.targetSystem] },
  ...
  _onMessage(payload) {
    ...
    if (
      ...
      this._clearTimeout();
      this._retries = 0;
      this.items.push(extractItem(payload.fields, payload.name, this.globalFrames));
      this._progress({ seq, count: this.count });
      const next = seq + 1;
      if (next >= this.count) {
        this._finish();
  ...
  _finish() {
    ...
    this._send('MISSION_ACK', { type: this.missionAccepted })
 ...
 * @param {Set<number>} globalFrames  generated MavFrame values with global coordinates
 * @returns {object}
 */
function extractItem(fields, messageName, globalFrames) {
  const item = {
