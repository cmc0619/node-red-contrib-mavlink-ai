...
class MissionUpload extends MissionWorkflow {
  ...
  _onMessage(payload) {
    ...
    if (payload.name === 'MISSION_ACK') {
       ...
       */
      if (result === MAV_MISSION_ACCEPTED && this.count > 0 && this._maxSeqSent < this.count - 1) {
        return;
      }
      this._clearTimeout();
...
function buildItemFields(item, seq, useInt = true, suppliedInt = useInt) {
  ...
  let y = item.y;
  /**
   * Raw x/y were authored for the item type the caller expected (`suppliedInt`,
   * the profile preference). The vehicle's request type wins over that
   * preference (#57), so when the two disagree the raw values must be
   * rescaled: degE7 int32 sent inside a float MISSION_ITEM would command a
   * point ~4e8 degrees away from home, and float degrees inside a
   * MISSION_ITEM_INT collapse to null island. Only finite values rescale
   * (NaN/undefined fall through to keepNanParam).
   */
  if (suppliedInt !== useInt) {
    if (x !== undefined && Number.isFinite(Number(x))) {
      x = useInt ? Math.round(Number(x) * 1e7) : Number(x) / 1e7;
    }
    if (y !== undefined && Number.isFinite(Number(y))) {
      y = useInt ? Math.round(Number(y) * 1e7) : Number(y) / 1e7;
    }
  }
