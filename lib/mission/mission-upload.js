...
class MissionUpload extends MissionWorkflow {
  ...
  _onMessage(payload) {
    ...
    if (payload.name === 'MISSION_REQUEST_INT' || payload.name === 'MISSION_REQUEST') {
      ...
      this._send(name, buildItemFields(item, seq, useIntForItem))
        .then(() => {
          this._setState(UPLOAD_STATES.WAITING_REQUEST);
    ...
    if (payload.name === 'MISSION_ACK') {
      ...
      /**
       * A stale/duplicate ACCEPTED can arrive right after MISSION_COUNT, before
       * the vehicle has pulled a single item — completing then would report
       * success for a transfer that never happened. Ignore an ACCEPTED until an
       * item has actually been requested (`_itemUseInt` is set on the first
       * MISSION_REQUEST(_INT)); a legitimate empty-mission clear (count 0) still
       * completes on its immediate ACCEPTED (#145). The count-retransmit timeout
       * stays armed so a genuinely stalled upload keeps progressing.
       */
      if (result === MAV_MISSION_ACCEPTED && this.count > 0 && this._itemUseInt === undefined) {
        return;
      }
      this._clearTimeout();
