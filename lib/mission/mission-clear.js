...
class MissionClear extends MissionWorkflow {
   ...
   */
  _start() {
    this._subscribe({ messageNames: ['MISSION_ACK'], sysid: this.targetSystem }, (msg) => this._onAck(msg.payload));
    this._sendClear();
  }
   ...
   */
  _onAck(payload) {
    if (!this._matchesTarget(payload) || !this._addressedToUs(payload)) {
      return;
    }
