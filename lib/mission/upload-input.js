...
function validateMissionItems(items, enums) {
  (items || []).forEach((item, seq) => {
    ...
    if (enums) {
      /** resolveEnumValue passes numbers/numeric strings through and maps a
       * known MAV_CMD name to its number; an unknown name comes back unchanged,
       * so Number(...) is NaN — reject it before the transfer starts. */
      const resolved = enumResolver.resolveEnumValue(enums, c);
