...
const { validateLatitude, validateLongitude, requireFinite, requireRange, requireIntRange } = require('../util/field-validation');
 ...
 * without standing up the whole node: canonical `items` resolution, a default
 * command for bare waypoint entries, and the stricter upload item validation
 * for issue #55. The pre-1.0 topic/`waypoints` aliases were removed (#283) —
 * the canonical inputs are `payload.action` and `payload.items`.
 */

/**
 * Resolve the upload item list from a payload. A simple waypoint entry —
 * `lat`/`lon` (and optional `alt`) with no explicit `command` — defaults to
 * `MAV_CMD_NAV_WAYPOINT` so `{ lat, lon, alt }` uploads a normal waypoint. The
 * advanced shape (explicit command/params/raw x-y/frame) passes unchanged.
 *
 ...
 * or non-array `items` is a wiring/config error (a typo'd or
...
function resolveUploadItems(payload, { allowEmpty = false } = {}) {
  ...
  if (p.items === undefined) {
    throw new MavlinkError(
      'MISSION_NO_ITEMS',
      'Mission upload requires an items array. Use the clear action to erase the on-vehicle mission.',
      {}
    );
  }
  const raw = p.items;
  if (!Array.isArray(raw)) {
    throw new MavlinkError(
      ...
      `Mission upload 'items' must be an array (got ${raw === null ? 'null' : typeof raw}).`,
      { field: 'items', type: raw === null ? 'null' : typeof raw }
    );
  }
...
/**
 * Raw x/y ride MISSION_ITEM_INT as int32, so a finite-but-huge value (1e20)
 * would still fail encoding only after MISSION_COUNT. The int32 bound is a safe
 * superset for both raw conventions: float degrees (|x| <= 180) and degE7
 * (|x| <= 1.8e9) both fit. z/alt are float fields — finite is enough.
 */
const INT32_MIN = -2147483648;
const INT32_MAX = 2147483647;
const INT32_COORD_FIELDS = ['x', 'y'];
/** Float coordinate fields sent raw — a finite number is required (no NaN sentinel). */
...
function validateMissionItems(items, enums) {
  const list = items || [];
  /**
   * Indexed iteration, not forEach: a sparse array (a Function node assigning
   * items[1] and leaving items[0] a hole) skips holes under forEach, so the
   * upload would pass preflight and then fail MISSION_BAD_SEQ mid-handshake
   * when the vehicle requests the missing seq. A hole lands here as undefined
   * and fails the object check like any other malformed item.
   */
  for (let seq = 0; seq < list.length; seq++) {
    const item = list[seq];
    if (!item || typeof item !== 'object') {
    ...
    for (const field of INT32_COORD_FIELDS) {
      if (item[field] !== undefined) {
        requireRange(item[field], field, INT32_MIN, INT32_MAX, { seq });
      }
    }
    ...
    for (const field of FLAG_FIELDS) {
      /** Natural JSON booleans are accepted for the 0/1 flags — they worked
       * before this preflight existed and the encoder coerces them to 1/0, so
       * only non-boolean values must be a 0/1 integer. */
...
module.exports = { normalizeUploadItems, resolveUploadItems, validateMissionItems };
