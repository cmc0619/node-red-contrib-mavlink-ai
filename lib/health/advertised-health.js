...
function normalizeAssertion(input, now) {
  ...
  // Fail-closed on overflow: a ttl_s so large that `now + ttl*1000` rounds to
  // Infinity would make the lease look like it never expires, so a `nominal`
  // claim could advertise MAV_STATE_ACTIVE forever. An unrepresentable expiry
  // is not a valid lease — reject it rather than let it never fail closed.
  if (!Number.isFinite(expires_at)) {
    const err = new Error('ttl_s is too large: the computed expiry overflows to a non-finite value');
    err.code = 'INVALID_HEALTH';
    throw err;
  }
  return { state, note, expires_at };
}
