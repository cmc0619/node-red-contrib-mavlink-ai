...
/**
 * Shared bounded tracking policy (#281): the identity-sweep threat model and
 * the eviction rationale live with the helper, one place for the registry and
 * the filter node both.
 */
const { boundedSet } = require('../util/bounded-map');
 ...
 */
/**
 * Render a rejected filter value for a BAD_FILTER message without breaking
 * the structured-error contract: JSON.stringify itself throws on BigInt and
 * circular values, which would replace the MavlinkError with a bare
 * TypeError exactly when the input is at its most malformed (#288 review).
 *
 * @param {*} value
 * @returns {string}
 */
...
function strictIdArray(value, field) {
  ...
  if (!Array.isArray(value)) {
    /**
     * Context carries the safe description, never the raw value: a BigInt or
     * circular original would blow up any consumer that serializes the
     * structured context (error payloads, logs) — the same failure the
     * message path already guards against (#288 review).
     */
    throw new MavlinkError(
      'BAD_FILTER',
      `Subscription filter ${field} must be an array of MAVLink ids (got ${describeBadValue(value)}).`,
      { field, value: describeBadValue(value) }
    );
  }
  return value.map((entry) => {
    ...
    if (!Number.isInteger(n) || n < 0 || n > 255) {
      throw new MavlinkError(
        'BAD_FILTER',
        `Subscription filter ${field} entry ${describeBadValue(entry)} is not an integer MAVLink id (0-255).`,
        { field, entry: describeBadValue(entry) }
      );
    }
    return n;
  });
...
function normalizeFilter(filter = {}) {
  if (filter.sysid !== undefined || filter.compid !== undefined) {
    throw new MavlinkError(
      ...
      { sysid: describeBadValue(filter.sysid), compid: describeBadValue(filter.compid) }
    );
  }
  return {
