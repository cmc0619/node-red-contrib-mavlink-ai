...
class OutboundQueue {
   ...
   *   so a udp transport can route to the addressed vehicle's endpoint)
   * @param {object} [opts]
  ...
  enqueue(buffer, priority = 2, meta = undefined, opts = undefined) {
    ...
    return new Promise((resolve, reject) => {
      ...
      /**
       * No sort here: ordering is owned by the drain loop, which re-sorts
       * immediately before every shift (both to pick the next item and to
       * honor aging across slow writes). A sort on enqueue was pure overhead —
       * nothing observes queue order between an enqueue and the next shift.
       */
      this._drain();
    });
  }
  ...
  /**
   * Splice out (without settling) every currently queued item whose
   * `coalesceKey` satisfies `predicate`. The one place that touches `_queue`
   * for a key-based eviction — `_removeCoalesced` (exact-key match, used by
   * enqueue-time `_dropSuperseded` and the public `dropCoalesced`) and the
   * public `dropCoalescedMatching` (predicate match, e.g. the #225
   * mixed-version-coalesce-key fatal-heartbeat stop) both build on this so
   * the splice logic exists exactly once.
   *
   * @param {function(*): boolean} predicate
   * @returns {Array<object>} the removed items (queue order, not settled)
   */
  _removeMatching(predicate) {
    const removed = [];
    for (let i = this._queue.length - 1; i >= 0; i -= 1) {
      if (predicate(this._queue[i].coalesceKey)) {
        removed.unshift(this._queue.splice(i, 1)[0]);
      }
    }
    return removed;
  }
  ...
  /**
   * Splice out (without settling) every currently queued item carrying the
   * given exact coalesce key. Thin wrapper over `_removeMatching` for the
   * common exact-key case (enqueue-time `_dropSuperseded` and the public
   * `dropCoalesced`).
   *
   * @param {*} coalesceKey
   * @returns {Array<object>} the removed items (queue order, not settled)
   */
  _removeCoalesced(coalesceKey) {
    return this._removeMatching((key) => key === coalesceKey);
  }
  ...
  _dropSuperseded(coalesceKey) {
    const removed = this._removeCoalesced(coalesceKey);
    let inheritedAt;
    for (const item of removed) {
      if (inheritedAt === undefined || item.enqueuedAt < inheritedAt) {
        inheritedAt = item.enqueuedAt;
      }
      item.resolve();
    }
    return inheritedAt;
  ...
  /**
   * Drop any still-queued item sharing this coalesce key, resolving its
   * pending promise — the same eviction `_dropSuperseded` performs on
   * enqueue, exposed publicly so a caller can evict a stale queued item
   * *without* enqueuing a replacement. Added for #225: when a health-driven
   * identity's heartbeat goes fatal, the tick skips its `send()` (so the
   * enqueue-time supersede never runs), but a heartbeat for that identity
   * already sitting in the queue from a prior tick must still be evicted —
   * otherwise a stalled/backlogged transport could flush a pre-fatal
   * heartbeat after the fatal assertion, violating "fatal stops the
   * heartbeat."
   *
   * @param {*} coalesceKey
   * @returns {number} how many queued items were dropped (0 if none matched)
   */
  dropCoalesced(coalesceKey) {
    const removed = this._removeCoalesced(coalesceKey);
    for (const item of removed) {
      item.resolve();
    }
  ...
  /**
   * Drop every still-queued item whose coalesce key satisfies `predicate`,
   * resolving each dropped item's pending promise — the predicate-based
   * sibling of `dropCoalesced` (exact key), sharing the same removal path via
   * `_removeMatching`. Added for #225 review: the mixed v1/v2 broadcast path
   * (#199) re-keys a coalesced frame per wire version
   * (`<baseKey>:v1` / `<baseKey>:v2`), so an exact-key `dropCoalesced` on the
   * bare base key misses those queued frames. A caller that needs to evict a
   * base key *and* every version-suffixed variant passes a predicate — see
   * the connection's fatal-heartbeat eviction helper.
   *
   * @param {function(*): boolean} predicate
   * @returns {number} how many queued items were dropped (0 if none matched)
   */
  dropCoalescedMatching(predicate) {
    const removed = this._removeMatching(predicate);
    for (const item of removed) {
      item.resolve();
    }
    return removed.length;
    ...
    try {
      while (this._queue.length) {
        ...
        try {
           ...
           * transport — and only if the item was not cleared mid-write. A
           * coalesced item is resolved in _dropSuperseded without ever being
           * written, and a cleared in-flight item was abandoned by teardown, so
           * neither calls onWrite (a debug trace must not count a superseded
           * heartbeat or a torn-down send as a real write).
           */
          if (item.onWrite && !item.cleared) {
            item.onWrite();
          }
          item.resolve();
        } catch (err) {
  ...
  clear() {
     ...
     * owns clearing _inFlight. Mark it cleared so a writer that resolves *after*
     * teardown does not fire onWrite — the send was abandoned, and a stale
     * post-teardown trace line would contradict the onWrite contract.
     */
    if (this._inFlight) {
      this._inFlight.cleared = true;
