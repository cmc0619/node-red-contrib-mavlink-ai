...
class OutboundQueue {
   ...
   * given coalesce key. The one place that touches `_queue` for a
   * key-based eviction — `_dropSuperseded` (enqueue-time supersede) and the
   * public `dropCoalesced` (external eviction, e.g. the #225 fatal-heartbeat
   * stop) both build on this so the splice logic exists exactly once.
   *
   * @param {*} coalesceKey
