 ...
 *
 * `generic` and `custom` firmwares have no standard param-def format, so they
 * resolve to "no source" and the editor keeps its free-text / numeric behavior
 * — graceful degradation is the default path, not a special case.
 */

const { parseApmPdefJson, parsePx4ParamsJson } = require('./param-def-parsers');

/**
 * Profile `profileType` (which doubles as the vehicle type) -> ArduPilot
...
module.exports = { resolveParamDefSource, ARDUPILOT_VEHICLE_DIR };
