 ...
 *
 * @param {object} enums    dialect enum index (resolves MavParamType members)
 * @param {string} [dialect]  dialect name, for the unavailable-enum error context
 * @returns {Map<number, {view: string, min: number, max: number}>} policy by
 *   MAV_PARAM_TYPE number
 */
function buildIntegerParamPolicy(enums, dialect) {
  // MavParamType is a common core enum — fall back to the core bundle when the
  // profile has no loaded dialect so param read/set/list still construct (#309
  // review); a present dialect still wins.
  const value = enums
    ? bindEnumValues(enums, { dialect: dialect || 'unknown', consumer: 'param' })
    : coreEnumValues({ consumer: 'param' });
  return new Map(
    Object.entries(INTEGER_PARAM_POLICY).map(([member, policy]) => [
      value('MavParamType', member),
      policy
    ])
  );
}
 ...
 * @returns {boolean}
 */
function isIntParamType(typeNum, integerPolicy) {
  return integerPolicy.has(typeNum);
}
 ...
 * @returns {number}
 */
function unionFloatToInt(floatValue, typeNum, integerPolicy) {
  const policy = integerPolicy.get(typeNum);
  if (!policy) {
    return floatValue;
  }
  _union.setFloat32(0, floatValue, true);
  return _union[`get${policy.view}`](0, true);
}
 ...
 * @returns {number}
 */
function unionIntToFloat(intValue, typeNum, integerPolicy) {
  const policy = integerPolicy.get(typeNum);
  if (!policy) {
    return intValue;
  }
  _union.setUint32(0, 0, true);
  _union[`set${policy.view}`](0, intValue, true);
 ...
 * @returns {number}
 */
function unionBitsToInt(bits, typeNum, integerPolicy) {
  const policy = integerPolicy.get(typeNum);
  if (!policy) {
    // No integer policy for this type — pass the value through, matching the
    // unionFloatToInt/unionIntToFloat guards (a non-int type is never reached
    // here today, but keep the four converters symmetric).
    return bits;
  }
 ...
 */
function unionIntToBits(intValue, typeNum, integerPolicy) {
  const policy = integerPolicy.get(typeNum);
  if (!policy) {
    return intValue >>> 0;
  }
 ...
 * @returns {object}
 */
function projectParam(fields, enums, integerPolicy, opts = {}) {
  ...
  if (bytewise && isIntParamType(typeNum, integerPolicy)) {
    /**
    ...
    out.param_value = Number.isFinite(bits)
      ? unionBitsToInt(bits, typeNum, integerPolicy)
      : unionFloatToInt(Number(fields.param_value), typeNum, integerPolicy);
...
class ParamWorkflow {
  ...
  constructor(opts = {}) {
    ...
    this.integerParamPolicy = buildIntegerParamPolicy(this.enums, this.dialect);
...
class ParamRead extends ParamWorkflow {
  ...
  _onValue(payload) {
    ...
    this._complete({
      topic: 'param/value',
      payload: projectParam(f, this.enums, this.integerParamPolicy, { bytewise: this._bytewise() })
    });
  }
}
...
class ParamSet extends ParamWorkflow {
  ...
  _start() {
    ...
    const range = typeNum == null ? undefined : this.integerParamPolicy.get(typeNum)?.range;
  ...
  _request() {
     ...
     */
    const isUnion = this._bytewise() && typeNum != null && isIntParamType(typeNum, this.integerParamPolicy);
    const wireValue = isUnion ? unionIntToFloat(this.value, typeNum, this.integerParamPolicy) : this.value;
    const extras = isUnion
      ? { exactFloatBits: { param_value: unionIntToBits(this.value, typeNum, this.integerParamPolicy) } }
      : undefined;
    this._send(
      'PARAM_SET',
      {
  ...
  _onValue(payload) {
    ...
    const param = projectParam(f, this.enums, this.integerParamPolicy, { bytewise: this._bytewise() });
    ...
    const applied =
      this._bytewise() && isIntParamType(typeNum, this.integerParamPolicy)
...
class ParamList extends ParamWorkflow {
  ...
  _finish() {
    ...
    const params = [...this.received.values()]
      .map((f) => projectParam(f, this.enums, this.integerParamPolicy, { bytewise }))
