...
const { requireEnumMember } = require('../protocol/protocol-values');
...
function buildIntegerParamPolicy(enums, dialect) {
  const context = { dialect: dialect || 'unknown', consumer: 'param' };
  return new Map(
    Object.entries(INTEGER_PARAM_POLICY).map(([member, policy]) => [
      requireEnumMember(enums, 'MavParamType', member, context),
      policy
    ])
  );
}
