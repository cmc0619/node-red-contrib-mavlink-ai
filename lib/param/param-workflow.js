...
/**
 * Validate an outbound param_id. The id is sent VERBATIM — the wire field is a
 * case-sensitive char[16], so forcing uppercase made a vehicle with lowercase
 * ids unaddressable (matching inbound echoes stays case-insensitive as a
 * leniency). An id longer than 16 chars would be silently truncated by the
 * serializer and then never match its echo, burning every retry into a bare
 * PARAM_TIMEOUT — reject it up front with a hint instead.
 *
 * @param {*} raw
...
class ParamWorkflow {
   ...
   *   the workflow's profile, not its default (`opts.profile` is a deprecated
   *   alias)
  ...
  _onValue(payload) {
    ...
    const matches =
      this.paramIndex != null
        ...
        : trimParamId(f.param_id).toUpperCase() === this.paramId.toUpperCase();
    if (!matches) {
      return;
    }
...
class ParamSet extends ParamWorkflow {
  ...
  _request() {
     ...
     */
    const isUnion = this.firmware === 'px4' && typeNum != null && isIntParamType(typeNum);
  ...
  _onValue(payload) {
    ...
    const f = payload.fields;
    if (trimParamId(f.param_id).toUpperCase() !== this.paramId.toUpperCase()) {
      return;
    }
