...
function resolveParamEncoding({ capabilities, firmware, enums, dialect } = {}) {
  const context = { dialect: dialect || 'unknown', consumer: 'param-encoding' };
  const bytewiseBit = BigInt(
    requireEnumMember(enums, 'MavProtocolCapability', 'PARAM_ENCODE_BYTEWISE', context)
  );
  const cCastBit = BigInt(requireEnumMember(enums, 'MavProtocolCapability', 'PARAM_ENCODE_C_CAST', context));
