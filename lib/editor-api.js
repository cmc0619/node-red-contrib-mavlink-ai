...
const { knownDialects, loadDialect } = require('./dialects/dialect-loader');
...
const EDITOR_COMPONENT_MEMBERS = ['AUTOPILOT1', 'CAMERA', 'GIMBAL', 'MISSIONPLANNER', 'ONBOARD_COMPUTER'];
const coreValue = coreEnumValues({ consumer: 'editor-api' });

/**
 * Resolve the request's `dialect` query param to a loaded dialect bundle.
 *
 * @param {object} req  the editor-API request
 * @param {boolean} [required=false]  when true a missing dialect throws
 *   DIALECT_REQUIRED; when false a missing dialect returns null
 * @returns {?object} the loaded dialect bundle, or null when not required and absent
 * @throws {MavlinkError} DIALECT_REQUIRED (missing + required), or the bundle's
 *   load failure re-wrapped as a MavlinkError (invalid/uncompilable dialect)
 */
function editorDialect(req, required = false) {
  const dialect = String(req.query.dialect || '');
  if (!dialect) {
    if (required) {
      throw new MavlinkError('DIALECT_REQUIRED', 'Flight-mode choices require the active Vehicle Profile dialect.');
    }
    return null;
  }
  ...
  if (!bundle.valid) {
    /**
     * loadDialect's invalid branch returns a plain `{ code, message, context }`
     * object; re-wrap as a MavlinkError so it carries a stack and satisfies an
     * instanceof check if handled anywhere beyond the immediate editorFailure.
     */
    throw new MavlinkError(bundle.error.code, bundle.error.message, bundle.error.context);
  }
  return bundle;
}

/**
 * Send the standardized editor-API failure envelope `{ ok: false, error }`.
 *
 * @param {object} res  the HTTP response
 * @param {Error} err  the thrown error (a MavlinkError carries code/context)
 * @param {string} fallbackCode  error code to use when `err` carries none
 * @returns {void}
 */
function editorFailure(res, err, fallbackCode) {
  res.status(400).json({
    ok: false,
    error: { code: err.code || fallbackCode, message: err.message, context: err.context || {} }
  });
}
...
function registerEditorApi(RED) {
  ...
  RED.httpAdmin.get('/mavlink-ai/protocol-values', guard, (req, res) => {
    try {
      res.json({
        ok: true,
        components: EDITOR_COMPONENT_MEMBERS.map((name) => ({
          name,
          value: coreValue('MavComponent', name)
        }))
      });
    } catch (err) {
      /** Keep the {ok,error} envelope its sibling routes use, rather than
       * falling through to Express's default handler on an enum lookup miss. */
      editorFailure(res, err, 'PROTOCOL_VALUES_FAILED');
    }
  });
  ...
  RED.httpAdmin.get('/mavlink-ai/modes', guard, (req, res) => {
    ...
    try {
      const bundle = editorDialect(req, true);
      const modes = knownModes({ firmware, vehicleType, enums: bundle.enums, dialect: bundle.name });
      res.json({ ok: true, firmware, vehicleType, modes });
    } catch (err) {
      editorFailure(res, err, 'MODE_CHOICES_FAILED');
    }
  });
  ...
  RED.httpAdmin.get('/mavlink-ai/param-choices', guard, (req, res) => {
    ...
    try {
      let enums = {};
      const bundle = editorDialect(req);
      if (bundle) {
        const customDialectPath = req.query.customDialectPath ? String(req.query.customDialectPath) : '';
      ...
      const result = resolveParamChoices(resolver, {
        firmware,
        vehicleType,
        componentType,
        enums,
        enumIndex: bundle ? bundle.enums : null,
        dialect
      });
      res.json({ ok: true, firmware, vehicleType, componentType, ...result });
    } catch (err) {
      editorFailure(res, err, 'PARAM_CHOICES_FAILED');
    }
  });
  ...
  RED.httpAdmin.get('/mavlink-ai/xml-catalog', guard, (req, res) => {
    try {
      ...
      /**
       * No baseDir in the response: it is an absolute server filesystem path,
       * nothing in the editor consumes it, and read-permission users don't
       * need the disclosure. The per-file `path` values remain — they are the
       * whole point (persisted into a profile's customDialectPath).
       */
  ...
  RED.httpAdmin.post('/mavlink-ai/xml-catalog/update', writeGuard, (req, res) => {
    ...
    /**
     * `repo` is interpolated into GitHub API/raw URLs: constrain it to the
     * owner/name shape so a crafted value can't inject extra path segments
     * into the server-side fetch. `ref` is resolved to a commit by the catalog
     * itself and follows the same character policy.
     */
  ...
  RED.httpAdmin.post('/mavlink-ai/param-catalog/update', writeGuard, (req, res) => {
    const body = req.body && typeof req.body === 'object' ? req.body : {};
    /**
     * The PX4 source URL is caller-supplied by design; require https so the
     * server-side fetch this triggers can't be pointed at plaintext or
     * non-HTTP schemes. (It remains an authenticated-admin capability — the
     * write permission gates it — but scheme discipline costs nothing.)
     */
    if (body.url !== undefined && !/^https:\/\//i.test(String(body.url))) {
      res.status(400).json({ ok: false, error: 'url must be an https:// URL.' });
      return;
    }
    const catalog = new ParamCatalog({ baseDir: paramCatalogBaseDir(RED) });
    catalog
