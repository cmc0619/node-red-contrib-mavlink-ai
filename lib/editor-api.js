...
function registerEditorApi(RED) {
  ...
  RED.httpAdmin.get('/mavlink-ai/protocol-values', guard, (req, res) => {
    res.json({
      ...
      components: EDITOR_COMPONENT_MEMBERS.map((name) => ({
        name,
        value: coreEnumMember('MavComponent', name, { consumer: 'editor-api' })
      }))
    });
  });
  ...
  RED.httpAdmin.get('/mavlink-ai/modes', guard, (req, res) => {
    ...
    if (!dialect) {
      res.status(400).json({
        ok: false,
        error: { code: 'DIALECT_REQUIRED', message: 'Flight-mode choices require the active Vehicle Profile dialect.' }
      });
      return;
    }
    const bundle = loadDialect(dialect, customDialectPath ? { customDialectPath } : {});
    if (!bundle.valid) {
      res.status(400).json({ ok: false, error: bundle.error });
      return;
    }
    try {
      ...
      res.status(400).json({
        ok: false,
        error: { code: err.code || 'MODE_CHOICES_FAILED', message: err.message, context: err.context || {} }
      });
    }
  });
  ...
  RED.httpAdmin.get('/mavlink-ai/param-choices', guard, (req, res) => {
    ...
    try {
      ...
      res.status(400).json({
        ok: false,
        error: { code: err.code || 'PARAM_CHOICES_FAILED', message: err.message, context: err.context || {} }
      });
    }
  });
