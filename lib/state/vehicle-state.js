...
function diffVehicleState(prev, next, at) {
  ...
  for (const s of next.health ? next.health.sensors : []) {
    ...
    // Fire on a healthy<->unhealthy flip for a known sensor, and also when a
    // sensor debuts already unhealthy (before === undefined): a sensor coming
    // online failed is a real edge for safety monitors. A healthy debut is not
    // alert-worthy and stays silent (from.healthy null marks "no prior read").
    const priorHealthy = before ? before.healthy : null;
    const debutUnhealthy = !before && !s.healthy;
    if ((before && before.healthy !== s.healthy) || debutUnhealthy) {
      push('sensor_health_change', { bit: s.bit, name: s.name, healthy: priorHealthy }, { bit: s.bit, name: s.name, healthy: s.healthy });
    }
  }
