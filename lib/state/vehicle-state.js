 ...
 */
/**
 * Sections owned by the autopilot component (compid 1). Messages of these
 * names from any other component are ignored (and must not create a vehicle).
 */
const AUTOPILOT_OWNED = new Set([
  'GLOBAL_POSITION_INT', 'HOME_POSITION', 'GPS_RAW_INT',
  'BATTERY_STATUS', 'SYS_STATUS', 'EXTENDED_SYS_STATE'
]);
...
class VehicleStateEngine {
  ...
  ingest(payload) {
    ...
    // HEARTBEAT and STATUSTEXT are accepted from any component; every other
    // section is autopilot-owned. Bail before allocating a vehicle record for an
    // autopilot-gated message from a non-autopilot component (e.g. BATTERY_STATUS
    // from a smart-battery component) so an ignored packet never spawns a ghost
    // vehicle with autopilot_seen:false and no components.
    const name = payload.name;
    if (name !== 'HEARTBEAT' && name !== 'STATUSTEXT'
      && !(AUTOPILOT_OWNED.has(name) && Number(payload.compid) === MAV_COMP_ID_AUTOPILOT1)) {
      return null;
    }
    ...
    if (payload.name === 'HEARTBEAT') {
      ...
      const f = payload.fields || {};
      // Cells 1-10 are in voltages; 11-14 (12S/14S packs) are in voltages_ext.
      // Sum both so large per-cell packs report the full pack voltage.
      const mainMv = sumCellVoltages(f.voltages);
      const extMv = sumCellVoltages(f.voltages_ext);
      const totalMv = mainMv === null && extMv === null ? null : (mainMv || 0) + (extMv || 0);
      const entry = {
        id: f.id,
  ...
  _snapshot(v) {
    ...
    return {
      ...
      mode: ap
        ? {
            name:
              firmware !== undefined && ap.custom_mode !== undefined && this.enums
                ? modeNameForCustomMode({ firmware, vehicleType, enums: this.enums, dialect: this.dialect }, ap.custom_mode)
                : null,
      ...
      landed: {
        ...
        // EXTENDED_SYS_STATE is streamed (PX4), so landed carries its own
        // freshness like position/gps/battery/health — a continuing HEARTBEAT
        // must not make a stale landed value look current. Absent until the
        // first message (ArduPilot never sends it): updated_at null, not stale.
        updated_at: v.landedAt !== undefined ? v.landedAt : null,
        stale: v.landedAt !== undefined ? now - v.landedAt > this.staleMs : false
      },
      position: v.position
...
function diffVehicleState(prev, next, at) {
  ...
  // Walk the union of prior and current sensor bits so every health edge is
  // caught: a healthy<->unhealthy flip, a sensor debuting already unhealthy
  // (before absent), and a sensor disappearing from all masks (after absent).
  // `healthy: null` marks "not observed" on whichever side is absent. A healthy
  // debut is deliberately silent — it is not alert-worthy and would otherwise
  // burst on the first SYS_STATUS for any vehicle booting with sensors present.
  const pSensors = new Map((prev.health ? prev.health.sensors : []).map((x) => [x.bit, x]));
  const nSensors = new Map((next.health ? next.health.sensors : []).map((x) => [x.bit, x]));
  for (const bit of new Set([...pSensors.keys(), ...nSensors.keys()])) {
    const before = pSensors.get(bit);
    const after = nSensors.get(bit);
    const pHealthy = before ? before.healthy : null;
    const nHealthy = after ? after.healthy : null;
    if (pHealthy === nHealthy || (!before && after && after.healthy)) {
      continue;
    }
    const name = (after || before).name;
    push('sensor_health_change', { bit, name, healthy: pHealthy }, { bit, name, healthy: nHealthy });
  }
