...
function decodePacket(bundle, packet, meta = {}) {
  ...
  /**
   * The wire version byte comes from the protocol instance the parser attaches
   * to every packet (its START_BYTE static), not header.magic — node-mavlink's
   * v1 parser never populates header.magic (it stays 0), so a v1 frame would
   * otherwise report raw.magic = 0 (#138).
   */
...
function resolveFlagArray(bundle, values, field, clazz) {
  ...
  const memberValues = members
    ? Object.entries(members)
        ...
        .map(([, v]) => v)
    : [];
  if (!isBitmaskEnum(memberValues, scopedEnum ? enumResolver.camelToScreaming(scopedEnum) : undefined)) {
