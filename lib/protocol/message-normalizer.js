 ...
 * strings. Everything else (numbers, `double`, char strings) passes through
 * unchanged. Never route a 64-bit value through `Number`, where precision above
 * 2^53 is silently lost.
 *
...
function resolveSingleValue(bundle, value, field) {
  // char/char[] fields carry text, not numbers or enum members. Enum/number
  // resolution must never run on them (issue #137): a string value that is
  // all digits ("123") or that collides with an enum member name ("GENERIC")
  // would otherwise become a Number, and node-mavlink's char serializer then
  // writes zero bytes — silently corrupting STATUSTEXT.text, PARAM_SET.param_id,
  // and any other char field. Pass the value through untouched.
