...
/**
 * A non-secret namespace for a passphrase's durable replay state (#101). The
 * signing key itself is `sha256(passphrase)`, so a domain-separated digest is
 * used instead: it cannot sign and does not reveal the passphrase, but a key
 * change yields a different scope so stale state is never inherited.
 *
 * @param {string} passphrase
 * @returns {string}
 */
function replayScopeKey(passphrase) {
  return crypto.createHash('sha256').update('mavlink-ai/replay-scope\0').update(passphrase).digest('hex').slice(0, 32);
}
...
class MavlinkCodec {
   ...
   * @param {object} [opts.replayStore]  durable anti-replay backing (#101); when
   *   omitted, replay protection is in-memory only (#100)
   */
  constructor({ bundle, version = 'auto', sysid = 255, compid = 190, signing = null, replayStore = null }) {
     ...
     * monotonic per-stream timestamp rule. In-memory by default (#100); durable
     * when the connection injects a replayStore (#101).
     */
    this._replay =
      this.signing && this.signing.verifyInbound && this._signKey
        ? new ReplayTracker({ scopeKey: replayScopeKey(this.signing.passphrase), store: replayStore })
        : null;
   ...
   * key) and then, as the signing spec requires, **anti-replay**: the frame's
   * timestamp must exceed the last accepted timestamp for its
   * `(sysid, compid, linkId)` stream. The check is in-memory by default (#100)
   * and durable across restarts when the codec was given a replay store (#101);
   * it runs whenever inbound verification is on — it is not separately toggled.
   *
