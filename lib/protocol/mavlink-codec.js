...
class MavlinkCodec {
  ...
  constructor({ bundle, version = 'auto', sysid = 255, compid = 190, signing = null }) {
    ...
    // likewise compid 0 is MAV_COMP_ID_ALL (the broadcast component address)
    // and must never be used as a *source* component id (#153).
    this.sysid = requireUint8(sysid, 'Source system id', 255, 1);
    this.compid = requireUint8(compid, 'Source component id', 190, 1);
    this._seq = 0;
  ...
  encode(name, fields, opts = {}) {
     ...
     * build a V2 protocol with the IFLAG_SIGNED incompatibility flag set (so the
     * header and CRC account for it), serialize, then append the signature block
     * — this is exactly node-mavlink's own sendSigned() sequence, just split so
     * the framed buffer can flow through our outbound queue/transport.
     *
    ...
    try {
      if (this.signsOutbound()) {
        ...
        let unsigned = ensureMinV2Payload(protocol.serialize(instance, this._nextSeq()), clazz);
      ...
      const versionTarget =
        opts.targetSystem !== undefined
          ...
          : merged.target_system !== undefined
            ...
            : merged.targetSystem;
      const protocol = this._makeProtocol(clazz.MSG_ID, versionTarget);
      const frame = protocol.serialize(instance, this._nextSeq());
      let out;
