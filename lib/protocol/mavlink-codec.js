 ...
 * exact patterns must be written into the frame after serialization. Re-extends
 * the payload when the v2 zero-trim cut it short of the field, and must run
 * before signing (the signature covers header+payload+CRC).
 *
 * @param {Buffer} frame  a serialized v2 frame
...
function applyExactFloatBits(frame, clazz, exact) {
  const payloadOffset = V2_PAYLOAD_OFFSET;
  let out = frame;
 ...
 * through a codec instance, which owns the dialect bundle and NOTHING else.
 * Every outbound frame is MAVLink 2; inbound v1 frames still decode (the
 * node-mavlink splitter/parser handle the 0xFE magic), so legacy peers are
 ...
 *  - the packet sequence counter, signing timestamps, and inbound replay
 *    memory live in the connection-owned {@link LinkState}
...
class MavlinkCodec {
  ...
  /**
   * Build and serialize an outbound message into a wire-ready MAVLink 2 frame.
   *
  ...
  encode(name, fields, opts = {}) {
    ...
    try {
      ...
      let out = ensureMinV2Payload(
        new MavLinkProtocolV2(sysid, compid).serialize(instance, link.nextSeq(sysid, compid)),
        clazz
      );
      if (opts.exactFloatBits) {
