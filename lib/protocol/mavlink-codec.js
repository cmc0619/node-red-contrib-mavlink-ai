...
/**
 * node-mavlink's internal PacketValidationResult enum is not exported; these
 * mirror its stable numeric values for {@link ResyncingPacketSplitter}.
 */
const PACKET_VALIDATION = { VALID: 0, INVALID: 1, UNKNOWN: 2 };

/**
 * MavLinkPacketSplitter with byte-wise resync on unvalidatable data (#153).
 *
 * The stock splitter treats INVALID (a known msgid whose CRC failed) and
 * UNKNOWN (a msgid with no CRC-extra entry) differently: INVALID resyncs at
 * the next byte, but UNKNOWN skips the full header-declared length — trusting
 * a length byte from a frame it could not validate at all. A false magic byte
 * inside line noise can therefore claim up to 280 bytes and swallow every
 * real frame in that window (four garbage bytes were observed swallowing ~14
 * valid frames). Unvalidatable data is treated like a CRC failure instead:
 * advance one byte and rescan, the same resync strategy as the MAVLink C
 * library and pymavlink.
 *
 * Cost: a genuinely-unknown message's payload is rescanned byte-wise for
 * frame starts — bounded, and only paid for traffic that could never be
 * decoded anyway, since the connection's merged CRC table covers every
 * message id any routed profile can decode.
 */
class ResyncingPacketSplitter extends MavLinkPacketSplitter {
  validatePacket(buffer, Protocol) {
    const result = super.validatePacket(buffer, Protocol);
    return result === PACKET_VALIDATION.UNKNOWN ? PACKET_VALIDATION.INVALID : result;
  }
}
...
class MavlinkCodec {
  ...
  createDecoder(onPacket, onError, opts = {}) {
    const splitter = new ResyncingPacketSplitter(
      {},
      ...
      const versionTarget =
        opts.targetSystem !== undefined
          ...
          : merged.target_system !== undefined
            ...
            : merged.targetSystem;
      const protocol =
        link.effectiveVersion(this.version, versionTarget) === 'v1' && !(clazz.MSG_ID > 255)
