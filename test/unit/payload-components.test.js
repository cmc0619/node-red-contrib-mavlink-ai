...
test('a camera verb honors an explicit per-message component (a standalone camera) (#155)', async () => {
  ...
  assert.strictEqual(collected[0][0].payload.target_component, minimal.MavComponent.CAMERA);
});
...
test('the node-configured component wins over the fallback for a gimbal verb (#155)', async () => {
  ...
  assert.strictEqual(collected[0][0].payload.target_component, minimal.MavComponent.GIMBAL);
});
...
test('gripper resolves grab/release strictly and fails an unknown action closed (#218)', async () => {
  ...
  assert.strictEqual(grab.collected[0][0].payload.fields.param2, common.GripperActions.GRAB);
  ...
  assert.strictEqual(rel.collected[0][0].payload.fields.param2, common.GripperActions.RELEASE);
