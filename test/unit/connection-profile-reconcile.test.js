...
// Codecs that frame ATTITUDE (id 30, ardupilotmega/common) from specific sysids.
// The `minimal` dialect has no ATTITUDE definition, so a system routed to a
// minimal profile cannot decode it and raises a structured decode error — that
// difference is how these tests prove which dialect a route actually used.
...
// GNSS_INTEGRITY (id 441) exists only in the development dialect, so its CRC
// extra is absent from the merged splitter table until a development profile is
// part of the effective routed set — it proves the merged CRC table updated.
...
// --- #117: routed profile edit/delete/recreate -----------------------------
...
test('the merged CRC table and decoder reset when a routed dialect set changes (#117)', async (t) => {
  ...
  const decoder1 = (() => {
    conn._transport.emit('data', attitudeFrom(1)); // force lazy decoder build
    return conn._decoder;
  })();
...
// --- #118: legacy name-resolution cache ------------------------------------
...
test('renaming a profile invalidates the old name and resolves the new one (#118)', (t) => {
  ...
  // Rename: recreate p2 with a new display name under the same id.
  profile(RED, 'p2', 'Renamed', 'ardupilotmega');
  // The cached id for 'Alt' now points at a node whose name no longer matches,
  // so the old name stops resolving and the new name resolves.
  assert.throws(() => conn.resolveProfile('Alt'), (err) => err.code === 'PROFILE_UNRESOLVED');
...
test('a name that becomes ambiguous fails PROFILE_AMBIGUOUS even after a cached unique result (#118)', (t) => {
  ...
  // so the next resolution re-scans and reports the ambiguity rather than
  // serving the stale unique result.
